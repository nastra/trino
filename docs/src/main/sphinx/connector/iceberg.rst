...
metastore service (HMS), AWS Glue, a REST catalog or Nessie. The catalog type is determined by the
``iceberg.catalog.type`` property, it can be set to one of ``HIVE_METASTORE`` / ``GLUE`` / ``JDBC`` / ``REST`` / ``NESSIE``.
