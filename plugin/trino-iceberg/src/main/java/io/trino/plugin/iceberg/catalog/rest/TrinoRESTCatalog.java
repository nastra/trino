        ...
        implements TrinoCatalog
{
    ...
    {
        Namespace n = Namespace.empty();

        if (namespace.isPresent()) {
            n = Namespace.of(namespace.get());
        }

        return sessionCatalog.listTables(convert(session), n).stream()
    ...
    {
        try {
            String cacheKey = format("%s-%s", session.getQueryId(), schemaTableName);
            return tableCache.computeIfAbsent(
                    cacheKey,
                    ignore -> sessionCatalog.loadTable(convert(session), toIdentifier(schemaTableName)));
        }
